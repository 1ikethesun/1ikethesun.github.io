...
방책 $\boldsymbol{\pi}$의 목적함수 $f$를 동시주변확률함수의 계열 $\varphi^{\boldsymbol{\pi}}0,\varphi^{\boldsymbol{\pi}}1,\dots$를 인수로 하는 함수 $\tilde{f}$를 이용해서, 
...
가 성립한다. 이때, $\varphi^{\pi^h_t}t(s,a|s_0)$

$= \varphi^{\pi^m_t}_t(s,a|s_0)$ 가 성립함을 배리법을 이용해서 보이겠다.
