...
앞으로 개념을 이해하는 동안, 행동집합은 상태에 의존하지 않는 단일의 집합 A로 다루도록 하겠다. 상태 $s$에 의해 선택 가능한 행동집합 $ A_s $가 모든 다른 경우에도 $ \mathcal{A}_1 $ $\cup \dots \cup $ $\mathcal{A}_{\vert \mathcal{S} \vert} = \mathcal{A} $
