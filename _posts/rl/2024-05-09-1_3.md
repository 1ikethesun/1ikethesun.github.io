...
$$
\vert \boldsymbol{\Pi}^{SD} \vert = \vert \Pi^d \vert = {\vert \mathcal{A} \vert}^{\vert \mathcal{S} \vert}
$$
...
$$
\vert \boldsymbol{\Pi}_{0:T}^{MD}\vert = \prod\limits_{t=0}^T \vert \Pi^d \vert = 
({\vert \mathcal{A}\vert}^{\vert\mathcal{S}\vert})^{T+1}
$$
