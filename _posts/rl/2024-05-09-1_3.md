...
$ \vert \boldsymbol{\Pi}_{0:T}^{HD}\vert = \prod\limits_{t=0}^T |\Pi_t^{h,d}| = \prod\limits_{t=0}^T {|\mathcal{A}|}^{|\mathcal{H}_t|} $ 
$ = \prod\limits_{t=0}^T {|\mathcal{A}|}^\{\mathcal{S}|^{t+1} |\mathcal{A}|^t}\} $
